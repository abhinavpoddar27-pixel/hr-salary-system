...
- **Date:** 2026-04-16
- **Branch:** `claude/session-start-MEzoy` (pushed to `origin/main` at `28c6029`)
- **Last commit:** `28c6029` feat(leaves): remove SL from UI and write paths
- **Task:** Three-commit leave-system overhaul (Apr 2026 policy): (1) Permanent-only accrual + DOJ-based CL pro-ration — `2e09606`. (2) Idempotency guard on `initCLOpening` — `5f36f48`. (3) Remove SL from UI + API write paths — `28c6029`. All three are now on `origin/main`.
- **Files changed this session:**
  - `backend/src/services/phase5Features.js` — added `LEAVE_ELIGIBLE_TYPES = ['Permanent']` config + `computeClEntitlement(doj, year)` helper (formula `7 - floor((effectiveMonth-1)/2)`, mid-month DOJ rolls forward). `runLeaveAccrual` employee SQL filtered by `employment_type IN (…)` via dynamic placeholders. CL block now seeds `leave_balances` via DOJ-based `ON CONFLICT DO NOTHING` before the existing mirror-usage logic runs. `initCLOpening` switched `UPSERT` → `ON CONFLICT DO NOTHING` on both `leave_balances` and `leave_accrual_ledger`, gated by a once-per-year `policy_config` guard (key `cl_seed_<year>_v1`). Guard write fires on any error-free run (incl. pre-guard-deploy "all rows already existed" case) so the 2026 transition is clean. `effectiveMonth` → `ledgerMonth` rename decouples ledger-placement from entitlement-value. `CL_PRORATA_BY_MONTH` lookup constant deleted. Commented-out TEMP TEST BLOCK at bottom covers 11 CL-entitlement cases.
  - `backend/src/routes/phase5.js` — 1 line: rewrote `POST /init-cl-opening` comment to describe idempotent behavior (old comment said "Safe to re-run" which was misleading); response message branches on `result.alreadyCompleted` to differentiate fresh-seed vs no-op re-run UX.
  - `backend/src/routes/leaves.js` — GET `/balances` SQL dropped the `'SL'` column from SELECT; GET `/balances/:code` response object dropped `SL:0`; added `['CL','EL']` whitelist validator to `POST /adjust` (was unvalidated); added same whitelist to `POST /bulk-adjust` (per-row continue on bad type); `PUT /:id/approve` balance-decrement and `DELETE /:id` cancel-refund allow-lists dropped SL. Three stale "SL has no balance model" comments rewritten.
  - `backend/src/routes/financeAudit.js` — `POST /corrections/apply-leave` validator `['CL','EL','SL']` → `['CL','EL']` + error message updated.
  - `backend/src/routes/employees.js` — two new-employee seed arrays (single-create line 301, bulk-import line 944) dropped SL so brand-new hires only get CL+EL balance rows.
  - `frontend/src/pages/LeaveManagement.jsx` — 7 surgical edits: `LEAVE_TYPES` dropped SL entry; balance fallback object `{CL:0, EL:0}`; apply-modal balance-chip array `['CL','EL']` + grid switched `grid-cols-3` → `grid-cols-2`; Leave Balances register table dropped `<th>SL</th>` + data cell, updated colSpan 8→7, TOTAL math now `CL + EL`; adjustment-form dropdown `<option value="SL">` removed; three stale `// SL` comments rewritten.
  - `frontend/dist/*` — rebuilt via `npm run build` (62 chunks re-hashed).
- **What was fixed/built:** (1) Leaves now accrue only for permanent employees, with DOJ-based CL pro-ration (7/6/5/4/3/2 by effective join month; mid-month DOJ rolls to next month; Dec mid-month rolls to next year → 0 CL). (2) `POST /init-cl-opening` is now truly idempotent — re-runs return `alreadyCompleted: true` instead of silently overwriting manual balance adjustments. HR can force a re-seed by deleting the `cl_seed_<year>_v1` row from `policy_config`. (3) SL is gone from the UI surface and all write paths: apply-leave dropdown, balance-chips, register table, adjustment form, `/adjust`, `/bulk-adjust`, `/approve`, `/cancel`, finance-audit apply-leave. 11 unit tests in-memory confirmed entitlement formula. Guard simulation confirmed manual-adjustment preservation across re-runs and clean independent seeding of different years.
- **What's fragile:** (a) `ON CONFLICT DO NOTHING` in `initCLOpening` means a re-run with a different `depMonth` arg has **no effect** — only the guard-absence path writes anything. If HR changes `deploymentMonth` after first seed, they must manually delete the guard row first. (b) `runLeaveAccrual`'s new `seedClBalance` call at the top of each employee's CL block runs once per employee per run — it's a `DO NOTHING` no-op after the first seed, but fires inside the monthly transaction, so a very large employee base means O(N) extra inserts per month. Harmless but visible in query-count dashboards. (c) `LEAVE_ELIGIBLE_TYPES = ['Permanent']` is a string-exact match — if HR ever types `'permanent'` or `' Permanent '` into `employees.employment_type`, that employee silently drops out of accrual. No trim/case-insensitive guard. (d) The "Apply Leave" modal's `grid-cols-2` layout assumes exactly 2 balance-tracked leave types. Adding a third (e.g. resurrecting SL, or introducing a new balance-backed type) requires toggling back to `grid-cols-3`. (e) `employees.js` seed arrays at lines 301 and 944 were trimmed, but neither path is the authoritative seeder — `initCLOpening` is. The two `employees.js` writes use hard-coded `opening=12` for CL with no DOJ-based pro-ration; they're vestigial paths HR likely doesn't hit post-bulk-import. Worth a future cleanup to delete entirely and route all CL seeding through `initCLOpening`. (f) Three commented-out test blocks now exist at the bottom of `phase5Features.js` (the 11-case CL-entitlement test from commit `2e09606`) — commented to prevent runtime noise, but re-comment status is fragile on future edits.
- **Unfinished work:** (a) Historical SL cleanup — SL rows in `leave_balances`, `leave_applications`, `leave_transactions` are still in the DB (intentional, for audit history). If HR later wants a cosmetic purge, that's a separate explicit DELETE prompt. (b) Railway production smoke tests (Test 1: GET /balances response, Test 2: POST /adjust with SL → 400) — spec'd in task but couldn't run from sandbox (HTTP 403 to Railway). Must be run by user after deploy. (c) Part 2 mobile-responsive pass (6 files + 2 judgment calls) queued from the 2026-04-11 session — still not touched this session. (d) `LateComingDeductionModal.jsx` verification against real production data — still queued. (e) `EmployeeProfile.jsx` AI Review "Regenerate" button shown even after error — pre-existing UX bug, not addressed.
- **Known issues remaining:** Pre-existing: (1) `DeptAnalytics.jsx` overtime tab field names not tested against real production data. (2) Local `main` branch diverged from `origin/main` (unrelated histories, no common ancestor) — **always push via `git push origin claude/session-branch:main`** never via local `main` checkout + merge. Confirmed this session by attempted ff merge that was impossible. (3) `EmployeeProfile.jsx` AI Review Regenerate button still shown on error. (4) SalaryComputation/DailyMIS/DayCalculation/Reports/Settings/EmployeeProfile/payslipPdf still render SL in historical contexts — intentional (audit history).
- **Next session should:** (a) Verify leave accrual end-to-end on Railway — HR clicks `/api/phase5/init-cl-opening?year=2026&deploymentMonth=1` once (should seed + write guard), a second click (should return `alreadyCompleted: true`). Confirm the guard row appears in `policy_config` as `cl_seed_2026_v1`. (b) Verify SL removal in the Leave Balances register UI (no SL column, TOTAL = CL+EL only). (c) Verify `POST /adjust` with `leave_type=SL` returns the new 400 error. (d) Verify historical SL data still renders in EmployeeProfile / Reports / payslip PDF. (e) Diagnostic SQL: `SELECT key, value FROM policy_config WHERE key LIKE 'cl_seed_%';` should show the guard row after first seed; `SELECT COUNT(*) FROM leave_balances WHERE leave_type='SL';` should be unchanged from pre-deploy.
