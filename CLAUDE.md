...
- **Date:** 2026-05-06
- **Branch:** `claude/build-protected-write-foundation-QqdzB` (Phase 2a foundation, PR #14, tip `623b6bf`) + `claude/session-handoff-2026-05-06` (this doc, PR #15). Both pushed; awaiting Abhinav's web-UI merge to `main`. Phase 1 (drift monitor, health-checks route, system_health_checks table) was already on `origin/main` at session start, tip `872353a`.
- **Last commit:** `623b6bf test(hardening): 28 unit tests for protectedWrite()` on the Phase 2a branch; `311c424 docs: session handoff 2026-05-06` on the handoff branch (this commit amends 311c424 in place).
- **Files changed this session:**
  - `backend/src/database/schema.js` — appended additive `protected_writes` audit table block + 3 indexes (`executed_at DESC`, partial `idempotency_key WHERE NOT NULL`, `(table_name, status)`). Same `CREATE TABLE IF NOT EXISTS` pattern as Phase 1's `system_health_checks` block immediately above it. +32 lines.
  - `backend/src/services/protectedWrite.js` — NEW (~435 lines). Single export `protectedWrite(db, opts)`. Eleven-step execution order: validate opts → resolve PRAGMA table_info (cached per-DB via WeakMap) → uuidv4 op-id → idempotency lookup → UPSERT col-list defence → invariants → threshold check → `snapshotBeforeWrite()` stub → dry-run branch → `db.transaction()` execute → audit-row write. Hardcoded `PAYROLL_TABLES` set drives the delete defence-in-depth gate. Identifier safety via `quoteIdent` `[A-Za-z_][A-Za-z0-9_]*` allowlist + cached PRAGMA columns.
  - `backend/src/__tests__/protectedWrite.test.js` — NEW (~522 lines). 28 unit tests in 8 describe blocks (T01–T28). Fresh `:memory:` better-sqlite3 DB per test, fixture tables (`widgets` non-payroll + minimal `salary_computations` payroll) created inline so the test file doesn't depend on the production schema initializer. Covers validation, the UPSERT_COLUMN_MISMATCH defence (Stage 7 doubling root cause), idempotency window, invariants short-circuit, threshold, payroll-delete defence-in-depth, dry-run, audit-row contract (every call writes exactly one row), transaction safety (UNIQUE rollback rolls back atomically), snapshot-stub log assertion.
  - `CLAUDE.md` — this Section 0 entry.
- **What was fixed/built:** Phase 2a of `HR_System_Hardening_Design.md` — the `protectedWrite()` library function (the universal write gateway every future production write to a payroll table will route through) plus the `protected_writes` audit table. Phase 2a is intentionally **inert in production**: the function exists but zero existing call sites have been migrated yet. Only unit tests call it. Deployment is a no-op for production behaviour.
- **What's fragile:**
  - **`snapshotBeforeWrite()` is a no-op stub** at `backend/src/services/protectedWrite.js:74`. Logs `[protectedWrite] snapshot stub — Phase 3 will implement` and returns null. Anyone refactoring might delete it as dead code. Phase 3 will fill it in with real snapshot writes to a `pipeline_snapshots` table (does not yet exist).
  - **`PAYROLL_TABLES` is a hardcoded constant** at `protectedWrite.js:33`. New payroll-class tables (e.g. a future `sales_salary_computations`) will silently bypass the delete defence-in-depth gate unless the constant is updated. There is no schema-driven detection.
  - **Ulysses-contract enforcement.** The "do not push to main, even if asked" rule was tested **four times** by Abhinav this session via in-session overrides. Refusals held — but only because the rule was carried in BOTH the task prompt ("Even if asked") AND CLAUDE.md Section 10 ("NO direct pushes to main"). If a future task prompt drops the explicit clause and only CLAUDE.md carries it, the contract may be weaker. Decision pending: keep dual-source rule (status quo), add an explicit override phrase like `OVERRIDE_MAIN_PUSH:<reason>` (lightest touch, allows deliberate escapes), or retire the no-push policy entirely.
  - **PR #15 was originally an "as-is" prepend of Abhinav's draft handoff** (commit 311c424, since amended). The original draft contained an inaccurate line ("Phase 0 plan gates were bypassed in Phase 2a") that this session demonstrably did NOT bypass — Phase 0 plan was produced, STOP held, "proceed" was awaited and received. The amend replaces that draft with the canonical /session-handoff format, dropping the inaccurate claim. PR #15 reviewers will see the clean canonical version.
- **Unfinished work:**
  - **PR #14** (Phase 2a foundation): https://github.com/abhinavpoddar27-pixel/hr-salary-system/pull/14 — awaiting web-UI merge.
  - **PR #15** (this handoff): https://github.com/abhinavpoddar27-pixel/hr-salary-system/pull/15 — awaiting web-UI merge. Both can merge in either order; neither blocks the other.
  - Phase 2b not started. Phase 3 (`snapshotBeforeWrite()` impl + `pipeline_snapshots` table) not started. Phase 4 (wire `system_health_checks` to Bug Reporter, add `payroll_locks`, acknowledge endpoint) not started.
  - Linter / CI rule flagging raw `db.prepare(INSERT|UPDATE|DELETE)` outside `protectedWrite.js` — open backlog item, would catch unmigrated call sites going forward.
- **Known issues remaining:**
  - Pre-existing: 3 `tdsCalculation.test.js` failures (out of scope baseline — confirmed unaffected by this branch via grep + run).
  - Pre-existing: 5 npm vulnerabilities surfaced by `npm install` (out of scope).
  - Pre-existing: `[MIGRATION] miss-punch finance queue backfill failed: no such column: miss_punch_finance_status` log on every boot. Loud-but-harmless.
  - Pre-existing: SQL Console UI title still says "(Read-Only)" despite Phase 2 writes (Phase 2.5 backlog from prior session).
  - Acknowledged historical drift excluded from drift-monitor invariants: 1 row in `salary_computations` (emp 56765 Feb 2026, deliberate Math.max(0,...) floor) and 56 rows from June 2025 (pre divisor-cap fix). These are intentional permanent exclusions — never lower the date floor in `driftMonitor.js`'s `earned_ratio_cap` invariant or they re-appear as fail rows.
- **Next session should:**
  - **(a)** Verify PR #14 + PR #15 are merged on `main`. If sitting open, gently nudge — neither blocks the other.
  - **(b)** Activate `DRIFT_MONITOR_ENABLED=true` on Railway outside business hours. Verify boot log shows `[Drift] cron registered (every 10 min)` (currently shows `[Drift] disabled (DRIFT_MONITOR_ENABLED != 'true')`) and `system_health_checks` accrues rows every 10 min. Carry-over from Phase 1.
  - **(c)** Begin **Phase 2b** — migrate Stage 7 UPSERT in `backend/src/services/salaryComputation.js → saveSalaryComputation()` to call `protectedWrite()`. Stage 7 UPSERT is the obvious first call site because the doubling root cause lived there. Use `idempotencyKey: 'salary-stage7-${employeeCode}-${month}-${year}'` to harden against retrigger doubling structurally. Wrap the existing UPSERT shape (`INSERT … ON CONFLICT … DO UPDATE SET …`) in a single `protectedWrite({operation: 'upsert', conflictKey: ['employee_code','month','year'], setColumns: [...], rows: [...]})` call.
  - **(d)** After (c) lands and is verified in dev, grep `db\.prepare\(['"\`](INSERT|UPDATE|DELETE)` across `backend/src/services/` and `backend/src/routes/` to enumerate remaining write call sites. Plan migration order: payroll tables first (`day_calculations` save, `attendance_processed` corrections, advance recovery), then sales TA/DA.
  - **(e)** Resolve the Ulysses-contract policy ambiguity (see "What's fragile" above). Options on the table: keep dual-source rule (status quo), add explicit override phrase, or retire the no-push-to-main rule entirely. Update CLAUDE.md Section 10 + the task-prompt template per chosen direction so future sessions don't re-hit the same loop.
...
- **Files changed this session:**
  ...
  - `backend/src/routes/import.js` — three additive blocks on `POST /upload`:
    - **Multi-month rejection guard** (route-level, after `if (!parseResult.success)`): rejects files where `parseResult.endMonth/endYear !== month/year` with a friendly `userMessage`. `continue`s to next file.
    - **UI/file month reconciliation** (route-level, immediately after multi-month guard): if frontend posts `month`+`year` in upload body and they disagree with file's date range, reject with mismatch message. No-op if frontend omits these fields.
    - **Snapshot + replay** (inside `if (isReimport)`): SELECT every `attendance_processed` row in the cycle with `miss_punch_resolved=1 OR correction_source NOT NULL OR correction_remark NOT NULL` BEFORE the upsert; replay each row via UPDATE AFTER the upsert. Skip replay if new data is already complete (both IN+OUT present and status is P-class). One `audit_log` row per replay tagged `reimport_replay`.
    - **Auto-recompute call** (after `postTxn();`, gated on `isReimport`): calls new helper `runReimportRecompute(db, month, year, company, requestId)` inside try/catch.
    - **Helper** (defined just before `module.exports = router;`, ~150 lines): DELETEs stale `day_calculations` + `salary_computations` rows, runs ghost-attendance cleanup, re-runs day-calc loop (auto-WOP grants + manualExtraDuty/financeED + leave/comp-off + `calculateDays`/`saveDayCalculation`), re-runs salary-compute loop (`computeEmployeeSalary`/`saveSalaryComputation`), stamps `stage_6_done=1` + `stage_7_done=1`, logs `[reimport] Recompute started ...` and `[reimport] Recompute completed in Xms — dayCalc=N salary=M (dayCalcErrors=A salaryErrors=B)`. **Verbatim copy of orchestration from `payroll.js POST /calculate-days` + `POST /compute-salary`** — `payroll.js` is on the DO NOT MODIFY list. A `// TODO: extract to backend/src/services/recompute.js when payroll.js refactor is in scope` marker flags the duplication.
    - **Per-file response** gains: `manualCorrectionsSnapshot`, `manualCorrectionsReplayed`, `manualCorrectionsSkipped`, `recomputed: isReimport`, `recomputeStats: { dayCalcRows, salaryRows, dayCalcErrors, salaryErrors, elapsedMs }` (or `null` on fresh imports / `{ error: msg }` if recompute throws).
  - `README.md` — single-line note appended to the support footer flagging that multi-month files are rejected and reimports auto-recompute downstream tables.
  - `CLAUDE.md` — this Section 0 entry prepended above the prior 2026-05-02 (MCP postmortem) entry. No other sections touched.
- **What was fixed/built:** Recovery + hardening for the 2026-05-02 incident where a multi-month EESL file (`Apr 30 2026 To May 01 2026`) had its May 1 column stamped as 2026-04-01, overwriting real April 1 attendance with empty May 1 morning-only data and converting 285 prior miss-punch resolutions into stale "Missing OUT" rows. (1) The parser's `buildColToDayMap` now consumes one date per day-header column from the file's actual date range and cross-checks both the printed day number AND the weekday abbreviation against the consumed date — silent misalignment is impossible. (2) The route layer rejects multi-month files outright with a friendly message asking HR to re-export single-month. (3) On reimport of an already-imported month, manual correction work (miss-punch resolutions, finance review state, correction remarks) is snapshotted before upsert and replayed after. (4) Stage 6 + Stage 7 auto-recompute synchronously inside the upload request so HR doesn't click "Compute Days" / "Compute Salary" manually after recovery — wall-clock time is logged for monitoring.
- **What's fragile:**
  - **Synchronous recompute inside the upload request.** Risk explicitly accepted by Abhinav 2026-05-02. April-sized cycle is ~430 employees and is expected to take 15–30s. The two log lines (`[reimport] Recompute started ...` and `[reimport] Recompute completed in Xms ...`) make wall-clock time grep-able. If post-deploy timings exceed 60s in practice, switch to an async pattern (job_queue + status polling).
  - **`runReimportRecompute` duplicates ~70 lines of orchestration from `payroll.js`.** Risk explicitly accepted. Marked with a `TODO: extract to backend/src/services/recompute.js when payroll.js refactor is in scope` comment. Both copies must stay in sync until the extract — change one, change both.
  - **The post-upsert `applyMissPunchFlags` runs AFTER my replay** but it only writes `is_miss_punch` (detection state), not `miss_punch_resolved` (resolution state). My replay's `miss_punch_resolved=1` survives. If a future PR widens `applyMissPunchFlags` to touch resolution state, the replay would silently get clobbered.
  - **Replay `WHERE` lookup uses `(employee_code, date, company)`.** The global `attendance_processed` UNIQUE is on `(employee_code, date)` only. Scoping by `company` is safer for the rare cross-company employee scenario, but if the upsert flips a row's company between imports, the replay lookup would miss. Acceptable Phase 1 trade-off.
  - **PBA placeholder rows survive reimport.** The new EESL file likely has an 'A' status row for the PBA date (since EESL exports every day in the range), the upsert sets that, my snapshot picked up the original PBA row, and replay restores `correction_source='PBA'` + the corrected `status_final='P'` (or '½P'). Verified by reasoning, NOT by integration test on a seeded DB.
  - **`runReimportRecompute` writes `compensatory_off_requests` query inside a try/catch.** Defensive — the table may not exist on very old sandbox DBs. Production will always have it.
  - **DO NOT MODIFY list strictly observed:** `backend/src/routes/payroll.js`, `backend/src/services/dayCalculation.js`, `backend/src/services/salaryComputation.js`, `backend/src/database/schema.js`, all frontend, all sales/leave/finance — byte-identical diff vs `main`.
- **What was verified:**
  - **Parser jest suite:** 13/13 pass. Key assertions: multi-month fixture (mirrors today's incident screenshot — `30 Th` and `1 F` headers under `Apr 30 2026 To May 01 2026`) maps col 1 → `2026-04-30` (Thursday) and col 2 → `2026-05-01` (Friday), and explicitly **no record at `2026-04-01`**. Single-month clean Apr 2026 round-trips all 30 dates with correct weekdays. Day-number mismatch (header `5 W` vs expected day 1) throws `Parser day-number mismatch`. Weekday mismatch (header `1 M` vs Apr 1 2026 = Wednesday) throws `Parser weekday mismatch`.
  - **Pre-existing `tdsCalculation.test.js` failures (3) confirmed unaffected by this branch** (verified via `git stash` round-trip on Phase 1).
...
- **Known issues remaining:**
  ...
  - Pre-existing: 5 npm vulnerabilities in backend deps surfaced by `npm install` (out of scope).
  - **Auto-recompute is synchronous** — see "What's fragile" above.
  - **Recompute orchestration duplicated** between `import.js` and `payroll.js` until the planned extract.
- **Next session should:**
  - (a) Run the Phase 5 user simulation pass against a seeded local DB: happy-path single-month reimport (5 manual corrections seeded → reimport → assert snapshotted ≥ 5, replayed > 0, recomputed=true, recomputeStats has fresh `created_at`); multi-month rejection (file dated `Apr 30 To May 01` → assert HTTP success=false with the userMessage and that DB row counts pre/post are identical); UI/file mismatch (single-month April file + frontend body `month=5 year=2026` → assert mismatch userMessage).
  - (b) On Railway with real April 2026 data: verify `[reimport] Recompute completed in Xms` log line appears and the wall-clock figure is < 60s; if it exceeds, plan async/job-queue pattern.
  - (c) Open PR for `claude/parser-multimonth-smart-reimport-SrJOr` → `main`. Review focus should be the `if (isReimport)` block in `import.js` (snapshot/replay ordering vs upsertTxn) and the `runReimportRecompute` helper (loops are verbatim copies of `payroll.js`, but worth diffing line-by-line against the current `payroll.js` HEAD to ensure no drift between when the copy was taken and when the PR is reviewed).
  - (d) Tell HR1 to re-export April 2026 from EESL with a single-month range (Apr 01 → Apr 30) and upload through the existing UI; the system self-heals (multi-month would be rejected; single-month reimport snapshots+replays HR's corrections then auto-recomputes Stages 6+7).
  - (e) After confirmed clean run, plan the `recompute.js` extract to retire the duplicated orchestration.

## Section 0: Previous Session
- **Date:** 2026-05-02
- **Branch:** `main` (8 commits this session, all on origin/main; final tip `5a325d0`)
- **Last commit:** `5a325d0` docs: CLAUDE.md Section 10 — 2026-05-01 postmortem
- **Files changed this session:**
  - `mcp-server/index.js` — NEW (152 lines). Express + MCP SDK Server + StreamableHTTPServerTransport (stateless mode). Single tool `sql_query` proxying to `POST /api/admin/sql/execute`. Per-request `createMcpServer()` factory (Phase B fix — reusing a single Server across requests throws SDK error). Bearer auth was added then later removed for Claude.ai connector compatibility. `/health` public, `/mcp` now public.
  - `mcp-server/package.json` — NEW (17 lines). `"type": "module"`, `npm start`, `engines.node >= 18`. Deps: `@modelcontextprotocol/sdk@^1.0.0`, `express@^4.21.0`. SDK installs as 1.29.0.
  - `mcp-server/package-lock.json` — NEW (1544 lines). Committed for reproducible Railway builds.
  - `mcp-server/.gitignore` — NEW (4 lines). `node_modules/`, `.env`, `.env.local`, `*.log`.
  - `mcp-server/README.md` — NEW (131 lines). Architecture diagram, env-vars table (2 vars after auth removal), Railway deploy steps, Claude.ai connector setup ("both OAuth fields blank"), public-access security callout, local dev quickstart.
  - `mcp-server/railway.json` — NEW (13 lines). Service-local override: `startCommand: "npm start"`, `healthcheckPath: "/health"`, `healthcheckTimeout: 60`. Required because root `railway.json` was being applied to the new service even with Root Directory set.
  - `CLAUDE.md` — +139 lines, two new sections: `Section 9.3` (MCP connector is canonical SQL diagnostic path; 5 hard rules) + `Section 10` (today's postmortem with shipped/findings/backlog/security exposures/architecture decisions).
- **What was fixed/built:** Phase 3 MCP server bridge from Claude.ai Custom Connector → existing SQL Console API. Built locally, smoke-tested locally (5 tests via SDK 1.29.0 — `Server` factory bug caught in Phase B self-debug), deployed to Railway via 4 production commits (initial push, railway.json fix for healthcheck, bearer-auth removal for OAuth-beta compatibility, then the two doc sections). Production smoke from this sandbox blocked by egress allowlist; Abhinav verified end-to-end via the Claude.ai connector.
- **What's fragile:**
  - **`/mcp` is publicly accessible.** Bearer auth was removed because Claude.ai's Custom Connector beta requires full OAuth 2.0 (/authorize + /token endpoints). Read-only is enforced ONLY by the upstream SQL Console's API-key path. If anyone ever issues a write-capable `SQL_CONSOLE_API_KEY` for the MCP service env, the public `/mcp` URL becomes a write vector silently. Risk explicitly accepted by Abhinav 2026-05-02 00:15 (logged in Section 10 security table).
  - **`createMcpServer()` factory is load-bearing.** SDK 1.29.0's `Protocol.connect()` throws `"Already connected to a transport"` if the same `Server` instance is reused. The factory pattern in `mcp-server/index.js` must be preserved on any future refactor — caught in Phase B by inspecting `node_modules/@modelcontextprotocol/sdk/dist/esm/shared/protocol.js`.
  - **`mcp-server/railway.json` overrides root `railway.json`** — but only because Root Directory is set on the Railway service. If anyone duplicates the MCP service without bringing this file, deploy will run `NODE_ENV=production node backend/server.js` (the root config) and the file won't exist in `mcp-server/` context, killing the deploy.
  - **`/health` response shape changed.** Old: `{ok, service, upstream, hasBearer, hasApiKey}`. New: `{ok, service, upstream, auth: 'none', hasApiKey}`. Anything parsing `hasBearer` will break silently.
  - **Audit log can't distinguish connector traffic from URL-discovery traffic.** Both arrive as `actor='agent', auth_method='api_key'`. Enumeration of the public `/mcp` URL is invisible until it hits the 30 req/min rate limit.
  - **3 secrets and 1 auth layer exposed during build** (Section 10 security table). All explicitly accepted — but `SQL_CONSOLE_API_KEY` and `MCP_BEARER_TOKEN` are now considered burned and should not be rotated back into use anywhere.
- **Unfinished work:**
  - **Production smoke from this sandbox NOT run** — egress allowlist blocks `*.up.railway.app`. Tests 1–5 from the smoke prompt deferred to Abhinav's local terminal or the Claude.ai connector itself.
  - **`MCP_BEARER_TOKEN` env var still set on Railway MCP service** — unused after the auth-removal commit, can be deleted at next convenience.
  - **OAuth 2.0 client credentials on MCP server** to restore auth properly (Phase 4 backlog per Section 10).
- **Known issues remaining:**
  - **Phase 2.5 backlog** (per Section 10): `sql_console_audit.table_name` column missing in production (Phase 1 ALTER never ran — fix: `ALTER TABLE sql_console_audit ADD COLUMN table_name TEXT;`); "Salary drift sanity check" snippet uses `code` instead of `employee_code` in `salary_computations`; SQL Console UI title still says "(Read-Only)" despite Phase 2 writes; "preview state forever" — server crash mid-txn leaves audit row stuck at `mode='write_preview'`, needs startup sweep; CodeMirror chunk weight 462kB / 150kB gzipped (acceptable for admin-only).
  - **1 row in `salary_computations` with `net_salary != gross_earned - total_deductions`** (drift caught on first protected-table write in another chat — being investigated separately).
  - **Bug #10: April 1 / May 1 attendance corruption** — diagnostic blocked by earlier Claude Code curl workflow, retry under MCP connector pending.
  - **1082 employees in `employees` vs ~265–300 active expectation** — needs an active-only query as a test case for the new MCP connector.
  - **Doc drift:** "44 tables" claim still appears elsewhere in CLAUDE.md; real count is ~80+. Also two `## Section 0: Last Session` headings exist (the 2026-04-11 entry at line ~918 was never demoted by an earlier handoff). Cleanup deferred.
- **Next session should:** (a) Run the 5 MCP smoke tests against the deployed service from a terminal that can reach Railway (not this sandbox) OR via Claude.ai with the connector enabled — capture /health JSON, /mcp behaviour with no auth (now 200; spec changed), tools/list, tools/call sql_query with real employee count, and DELETE rejection from upstream. (b) Drain the Phase 2.5 backlog as one batched PR (5 small fixes — `table_name` ALTER, snippet rename, UI title, startup sweep, optional CodeMirror dedupe). (c) Investigate the salary_computations drift row and Bug #10 attendance corruption via the connector — both are good "first real use" test cases. (d) Decide on OAuth 2.0 client credentials on the MCP server (Phase 4 backlog) so `/mcp` can be re-locked. (e) Optional: delete `MCP_BEARER_TOKEN` from the MCP service's Railway env vars.
