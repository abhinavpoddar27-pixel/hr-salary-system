...
- **Date:** 2026-05-01
- **Branch:** `claude/sql-corruption-diagnostic-9rdrD` (created this session, 0 source commits — diagnostic only, no code changes)
- **Last commit:** `31a2a44` feat(sql-console): Phase 2 — bounded writes + snapshot + restore (#10) — pre-existing on `origin/main`, branch was forked from this tip and never advanced.
- **Files changed this session:** None (working tree clean throughout). CLAUDE.md updated this turn with this handoff only.
- **What was attempted:** Run an 8-query read-only SQL diagnostic for Bug #10 (April 1 / May 1 attendance corruption) via the SQL Console API. Queries scoped: Q1 monthly_imports timeline (Apr+May 2026), Q2a–c attendance_processed sample on the 3 affected dates + April 30 control, Q3a–b manual-work-at-risk + downstream-already-computed check, Q4a–b raw-import history for April 1 (which import wrote it, what the original raw rows looked like). All 8 queries are SELECT-only, no writes proposed. Full SQL preserved in this turn's user message and re-runnable verbatim.
- **What blocked it:** `SQL_CONSOLE_URL` and `SQL_CONSOLE_API_KEY` were unset in this session's shell env at the moment the curl loop would have run. User shared their local `~/.zshrc`/`~/.bashrc` snippet via screenshot at end-of-session (URL = Railway production domain, API key = 64-char hex), but by then the session was wrapping up and the diagnostic loop never executed. No partial output, no /tmp artifacts, no audit rows on the production `sql_console_audit` table for this session.
- **What's fragile:** Nothing changed, so nothing new is fragile. Pre-existing fragilities from §0 prior entries (env-gate string-equality, cache-hit /schema doesn't audit, JWT bypasses rate limit, etc.) are unchanged.
- **Unfinished work:** The Bug #10 diagnostic itself. The 8 queries are still pending and need to be run from a shell where both env vars are exported. CLAUDE.md §0 prior entry (2026-04-30) already notes "(c) From a local terminal, `export SQL_CONSOLE_URL=...` and `SQL_CONSOLE_API_KEY=...` and verify the agent path with `/sql-query`" — the values shown in the user's screenshot are presumably already set in their personal shell, just not loaded into this agent's `bash` invocation.
- **Known issues remaining:** Bug #10 (April 1 / May 1 attendance corruption) is undiagnosed — the user reports something visibly wrong on those two dates and a control day (April 30) was added to the query set as a sanity baseline. No hypothesis was offered or accepted; the diagnostic is read-only and explicitly told the agent NOT to propose remediation.
- **Next session should:** (a) Verify env vars are loaded — `echo $SQL_CONSOLE_URL` should print the Railway domain, `echo ${#SQL_CONSOLE_API_KEY}` should print 64. If unset, re-source `~/.bashrc` / `~/.zshrc` BEFORE the agent starts. (b) Re-paste the 8-query diagnostic prompt from this session's user message verbatim and let the agent run all 8 in order, reporting parsed JSON only. The prompt has explicit "do NOT propose follow-up writes" and "the human will interpret" guardrails — preserve those. (c) After the human reads the output, decide whether the next action is a re-import (Stage 1 reupload of April EESL XLS), a targeted Phase 2 SQL Console write (UPDATE with /preview→/commit, mandatory remark, snapshot for `attendance_processed`), or a manual correction via the existing Stage 5 corrections UI. (d) Treat any audit-row id from the eventual diagnostic run as the citation anchor per §8 rule "every /ship report's table/file/column claims must come from grep/find/sqlite3 queries executed in the current session" — quote `audit_id` not memory.
