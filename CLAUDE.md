...
- **Files created (backend):**
  ...
  - `backend/src/services/bugReportResurrect.js` — boot-time scan, two buckets: (A) `claude_run_status='pending' AND transcription_status='success'` → re-run Claude only; (B) `transcription_status='pending' AND sarvam_job_id IS NULL` → re-run full pipeline. Both filtered by `datetime(created_at) > datetime('now','-24 hours')`. Uses `setImmediate` fire-and-forget. Bucket C (batch jobs stuck) is left to `sarvamBatchPoller`.
...
- **Files modified:**
  ...
  - `backend/src/database/schema.js` — 1 new table: `bug_reports` (`CREATE TABLE IF NOT EXISTS`), PK = `id`. 5 supporting indexes on `admin_status`, `created_at DESC`, `reporter_username`, `(admin_status, created_at DESC)`, and a partial index on `sarvam_job_id WHERE sarvam_job_id IS NOT NULL`. Status vocab: `transcription_status IN ('pending','rest_sync','batch_queued','batch_polling','success','failed','skipped')`, `claude_run_status IN ('pending','success','failed','skipped')`, `admin_status IN ('new','triaged','in_progress','resolved','wont_fix','duplicate')`. Compound CHECK: `input_method` vs `audio_path`/`user_typed_comment` consistency. Added `policy_config` seeds: `bug_report_extraction_prompt` (full Claude prompt), `bug_report_extraction_prompt_version` (string tag for cache invalidation), `bug_report_known_pages_json` (pathname→canonical-page-name map). Plan v3 also calls out `bug_report_actions` and `bug_report_analysis_runs` audit tables — **those were NOT implemented**; state-transition audit and retry tracking are currently only what's inline on the `bug_reports` row itself (`admin_status`, `transcription_status`, `claude_run_status`, `resolved_at`/`resolved_by`, `transcription_error`, `claude_error`). Add those tables before relying on a retry counter or a per-action audit trail.
...
- **What's fragile:**
  ...
  - `bugReportResurrect` scans the two buckets above and re-enqueues via `setImmediate`. **There is NO retry counter** — if Sarvam returns 5xx persistently, the same row will be re-queued on every boot until it ages past 24 hours. The only guard is the `datetime(created_at) > datetime('now','-24 hours')` cutoff. If you see the same row retrying indefinitely, bump the row's `transcription_status`/`claude_run_status` to `'failed'` manually, or implement `bug_report_analysis_runs` to track attempts.
...
- **New env vars (already on Railway per user):** `SARVAM_API_KEY`, `SARVAM_WEBHOOK_SECRET`, `ANTHROPIC_API_KEY` (already in use for Salary Explainer). **Note:** `sarvamWebhookVerify.js:45` and `sarvamTranscription.js:31` both read `process.env.SARVAM_WEBHOOK_SECRET`. The `policy_config` row `bug_report_sarvam_webhook_secret` seeded by `schema.js` on first boot is NOT read by any code path — it's a dead seed removed in the same session. Going forward the only live source of the secret is the Railway env var.
...
- **Data-exposure cleanup shipped this session:**
  - `.gitignore` — added `backend/data/*.db`, `backend/data/*.db-journal`, `backend/data/*.db-wal`, `backend/data/*.db-shm` alongside the existing `data/*` rules. The existing rules never matched because the real DB lives at `backend/data/*`, not `data/*`. Older commits (including `20a290d`) still contain `backend/data/hr_system.db`, `hr_system.db-wal`, `hr_system.db-shm`, and a nightly backup at `backups/hr_salary_2026-04-11_17-42.db` — these remain in git history until a separate history-rewrite pass (out of scope for this session but flagged below).
  - `backend/src/database/schema.js` — removed the dead `bug_report_sarvam_webhook_secret` seed block (lines 2129–2142 in the old file). It generated a 64-char hex secret and inserted it into `policy_config` on first boot, but no code ever read it. Left in place it would have landed in the next nightly backup commit.
- **Next session should:** (a) Verify on Railway that Sarvam webhooks arrive (`POST /api/bug-reports/sarvam-webhook`), batch poller recovers any missed ones, and Claude extraction runs end-to-end with real audio. (b) Monitor `claude_run_status='failed'` and `transcription_status='failed'` row counts in the first week — if >1%, tune the Sarvam polling timeout or Claude prompt. (c) If HR hits the 10/hr limit during an incident, consider bumping to 20 or making the limit per-IP rather than per-user. (d) Consider adding an admin "Export all as JSON" button for analytics / ticket handoff to external systems. (e) **Repo hygiene / remaining leak vector**: decide whether to `git rm --cached` + history-rewrite (BFG) the committed DB + WAL/SHM + nightly backup. Everything in those files (user password hashes, employee PII, all `policy_config` values) is already on GitHub at commit `20a290d` and the backup commits. A rotation + history rewrite is the only clean fix. Not blocking, but don't push any new backups (the cron commit path was not touched this session).
