...
- **Date:** 2026-04-29
- **Branch:** `claude/review-salary-findings-p4WV2` (no code changes this session — production data correction operation, blocked at execution-path discovery)
- **Last commit:** `ccc9ad9` docs: session handoff 2026-04-28 (yesterday's handoff; today's session adds zero new commits other than this handoff)
- **Files changed this session:**
  - `CLAUDE.md` — this Section 0 entry only. No code touched. Pure read-only diagnostics against production DB via the admin Query Tool, plus a failed attempt to execute DDL via `railway run sqlite3`.
- **What was attempted/built:** Production data correction for the 2026-04-28 Mar→Feb cycle misattribution. The "Sales feb attendance" file uploaded yesterday was attributed to Mar 2026 by the system but represented the Feb 2026 cycle (Jan 26 – Feb 25). Goal: clean delete of all Mar 2026 Indriyan rows so HR can re-upload via Fix D's cycle picker explicitly to Feb 2026. Three plan iterations this session: (a) v1 UPDATE-based migration (Mar→Feb) cancelled by user before any writes; (b) v1 DELETE-based plan started but flagged as wrong-scope; (c) v2 DELETE-based plan with corrected scope authored by user, Steps 1-2 (read-only diagnostics) executed cleanly, **Step 3 (backups) BLOCKED** at execution path. Total rows that need deletion: **848** across 5 sales_* tables (2 / 346 / 164 / 168 / 168).
- **What's fragile:**
  - **Mar 2026 Indriyan misattribution remains unresolved in production.** 848 rows still wrongly attributed across 5 tables: `sales_uploads` (2 rows — two upload attempts today), `sales_monthly_input` (346 rows across upload_ids 1+2; the table is keyed on `upload_id` not the (employee,cycle) tuple, so re-uploads accumulate dupes per design), `sales_salary_computations` (164 rows — UPSERT-deduped per employee), `sales_ta_da_computations` (168 rows — Phase α auto-trigger fired during salary compute), `sales_ta_da_monthly_inputs` (168 rows — same Phase α path).
  - **`sales_diwali_ledger` and `sales_payslips` are NOT in scope.** First was dropped in the Q5 reversal hotfix (2026-04-21); second never existed. Yesterday's v1 prompt incorrectly listed both. v2 prompt corrected this. Any future correction prompt that lists either is wrong.
  - **TA/DA tables `sales_ta_da_computations` / `sales_ta_da_monthly_inputs` ARE in scope** despite the v1 prompt's "no TA/DA computed for Mar 2026" assertion. Phase α auto-trigger inside `POST /api/sales/compute` fires unconditionally after salary compute; if a Mar 2026 compute happens, the TA/DA tables get populated. Any future production-correction plan touching sales cycle data must include them.
  - **Foreign keys discovered (Step 2):** only one matters for DELETE order — `sales_monthly_input.upload_id → sales_uploads.id`, `on_delete=NO ACTION`. Must DELETE from `sales_monthly_input` BEFORE `sales_uploads`. TA/DA tables FK only to `sales_employees` (master, not in scope). `sales_salary_computations` has no declared FK to anything in scope. Confirmed DELETE order: ta_da_computations → ta_da_monthly_inputs → salary_computations → monthly_input → uploads. All `on_delete=NO ACTION` — no cascade behavior, every DELETE is explicit.
  - **`sales_uploads.company` is fully-qualified `'Indriyan Beverages Pvt Ltd'`** with no string variants. Yesterday's "0 rows" diagnostic against this filter was caused by Query Tool's English-mode AI translator stripping the `'Pvt Ltd'` literal from the WHERE clause — confirmed today by re-running in Paste SQL mode. **Lesson: always use Paste SQL tab for any query with string literals; English mode silently mangles them.** This applies broadly, not just to this correction.
  - **Query Tool is SELECT-only** — validator at `backend/src/routes/queryTool.js` blocks `INSERT/UPDATE/DELETE/DROP/ALTER/CREATE/TRUNCATE/REPLACE/ATTACH/DETACH/PRAGMA/VACUUM/REINDEX`. No way to run backups, DELETEs, or audit_log INSERTs through it. Documented in CLAUDE.md but discovered fresh today as a blocker.
  - **`railway run sqlite3 backend/data/hr_system.db` does NOT work** for production DB access. Got "unable to open database file" — `railway run` proxies env vars from production into a local shell but does NOT mount the production container's filesystem. Confirmed by attempt today. Alternative paths (untested): `railway shell` for interactive shell into the running container, Railway dashboard's "Run command" feature, building a one-off admin write endpoint that bypasses the validator, or pulling the DB locally → operating → uploading back (risky due to concurrent writes from background schedulers + sessionTracker + audit_log). None tried/established.
  - **Backup-table naming convention `_backup_2026_04_28_sales_*`** is committed in the v2 plan. If the actual deletion happens 2026-04-29 or later, the date stamp in the backup-table names becomes misleading. Acceptable since the misattribution date is 2026-04-28 (the operation it backs up); but worth flagging for the rollback plan to remain readable.
- **Unfinished work:**
  - **Steps 3 → 6 of the v2 deletion plan (backups, DELETE transaction, post-verify, user re-upload as Feb 2026) — all blocked on execution path.** Steps 1 and 2 (read-only) are complete with results matching expectations exactly: 5 in-scope tables confirmed, 848 rows confirmed, FK order confirmed, company string verified.
  - **No execution path established for production DDL/DML.** This is the single blocker. Next session must resolve this before any progress.
  - **CLAUDE.md update for the post-deletion bug-runbook entry** (the v2 prompt's final note: "sales_monthly_input accumulates rows across upload attempts — re-uploading the same cycle creates dupes; this is by design but should be flagged in HR runbook") — deferred until deletion actually completes.
- **Known issues remaining:**
  - **Mar 2026 Indriyan misattribution** — still 848 rows wrong across 5 sales_* tables; no NEFT exported, no payments — clean rollback when execution path resolves.
  - **Query Tool's English-mode literal-mangling** — silently rewrites string literals in user-provided SQL. Affects any diagnostic query touching company strings, status enums, audit-log filters, or any literal value with multiple words. Workaround: always use the Paste SQL tab. Long-term fix would be to disable LLM rewriting when the input is already valid SQL, but that's a code change with its own scope.
  - **Production DB execution path** — there's no documented runbook for executing destructive SQL against production. The /ship workflow assumes code changes; data corrections via SQL aren't covered. Worth a separate runbook entry once the path is established.
  - Pre-existing items inherited unchanged from yesterday's handoff: Phase 4 fix E (excess `days_given` validation) still pending; "most-recent-cycle-with-data" default for the cycle picker still deferred; URL persistence for cycle picker not implemented; `RATE_FIELD_LABELS` dead import in `SalesTaDaApprovals.jsx:15`; Phase 3 feature branch `claude/sales-phase3-tada-compute-nqe1h` still on origin awaiting smoke test; most master employees still NULL `ifsc`.
- **Next session should:** (a) **Establish a production DDL/DML execution path** — try `railway shell` first (drops you into an interactive shell inside the running container; from there `sqlite3 /app/backend/data/hr_system.db` should work since the volume is mounted); if that fails try Railway dashboard's "Run command" or building a one-off admin write endpoint. (b) Once execution path works, **run the validated v2 deletion plan** verbatim — Steps 3 (5 CREATE TABLE backups + verification SELECT in Query Tool), Step 4 (single transaction with DELETE × 5 + audit_log INSERT, in the confirmed FK-safe order), Step 5 (verification SELECTs in Query Tool), Step 6 (user instructions for re-upload as Feb 2026 via Fix D's cycle picker). The full SQL is in conversation transcripts and the v2 prompt — do not re-derive. (c) After deletion completes, **append a runbook note to CLAUDE.md** about `sales_monthly_input` accumulating rows across upload attempts (the user explicitly requested this — re-uploading same cycle creates dupes per FK design, by-design behaviour but worth flagging for HR). (d) Drop the `_backup_2026_04_28_sales_*` tables after 30 clean days (deferred reminder). (e) Optional: open a separate ticket for "establish production DDL/DML runbook" so the next correction has a documented path; and a separate ticket for "Query Tool English-mode silently mangles SQL literals" since this caused yesterday's wasted diagnostic cycle.
