...
- **Date:** 2026-05-01
- **Branch:** `feat/sql-console-phase-2-writes` (merged to `main` via PR #10 — merge commit `31a2a44d5ede4bcc52127ccd989fb8a3690332e8`, "Create a merge commit" preserved all 8 source commits). Origin/main has since advanced 5 more commits (parallel MCP server work landed at `0107ca7`); our 8 are inside main's history regardless.
- **Last commit (this session):** `11fc48d` docs(sql-console-p2): CLAUDE.md Section 9.1 (write flow). Eight commits in PR #10 in topological order: `04be101` (Phase A validator+txn registry) → `4d6730f` (Phase B+C preview/commit/rollback) → `a688a23` (chore: gitignore for `backend/_p2_*`) → `de44c65` (Phase D snapshot restore) → `f56c3c1` (Phase E rate limit + DDL gate consolidation) → `5d78ae6` (Phase F frontend write flow) → `7d3e847` (Phase H hotfix: audit row pre-BEGIN INSERT) → `11fc48d` (docs Section 9.1).
- **Files changed this session:**
  - `backend/src/routes/sqlConsole.js` — +990 lines (587 → 1577). New: `validateWriteSql` (6-layer), `txnRegistry` Map, `beginManualTxn`/`commitManualTxn`/`rollbackManualTxn`, `captureRowsBefore`/`captureRowsAfterInsert`/`captureRowsAfterUpdateByPk`, `sanitizeRowForJson`, `generateInverseStatements`, `getPrimaryKeyColumns`, `detectSchemaDrift`, `runDriftCheck`, `invalidateReadonlyDb`, `ensureWriteSnapshotsTable`, `writeRateLimiter`. New endpoints: `POST /preview`, `POST /commit/:txn_id`, `POST /rollback/:txn_id`, `POST /snapshot/:audit_id/restore`. Existing: `ensureAuditTable` extended with `restored_from_audit_id` ALTER, `logAuditRow` signature extended (additive), `mountSqlConsole` adds DDL-token boot check + Phase 2 status log line.
  - `frontend/src/pages/SqlConsole.jsx` — +657 lines (420 → 1077). New: `detectWriteIntent` + `isDdlIntent` + `changedColumns` + `formatCellValue` + `stripCommentsAndStringsClient` helpers. New components: `PreviewModal` (remark + DDL token + UNSCOPED retry), `DiffView` (Date.now()-anchored countdown, op-aware diff), `DiffTable`+`SimpleRowList` (row-level diff renderers), `ConfirmModal` (re-type-remark guard), `SnapshotRestoreFlow` ("Snapshots ▾" toolbar dropdown). New state: `writeFlowStep`, `pendingRemark`, `pendingDdlToken`, `pendingAcceptUnscoped`, `previewData`, `confirmRemarkInput`, `committing`, `snapshotsList`, `lastCommitToast`, `now`. `runQuery` kept untouched; new `handleRun` routes writes via `detectWriteIntent`.
  - `CLAUDE.md` — +36 lines, new "Section 9.1: SQL Console Phase 2 — Bounded Writes (May 2026)". Section 9 (Phase 1) lost the now-stale "Phase 2 NOT in this build" line, replaced with cross-reference to 9.1.
  - `.gitignore` — +2 lines: `backend/_p2_*` (local-only test scaffolding, never deployed).
  - `frontend/dist/*` — rebuilt; new `SqlConsole-DUkK2XL7.js` chunk (462 kB raw / 150 kB gzip; was 444 kB / 146 kB in Phase 1 — modal markup + DiffTable adds ~18 kB).
- **What was fixed/built:** Phase 2 of the SQL Console — bounded writes with mandatory remark + preview → commit/rollback flow + snapshot table + restore endpoint + drift check on salary cascade tables. Per-actor 10/hour rate limit on /preview and /restore. Optional `SQL_CONSOLE_DDL_TOKEN` env var enables CREATE/ALTER/DROP behind a header gate. Frontend gets a 4-modal write flow with a Snapshots dropdown for restore.
- **What's fragile:**
  - **Audit row INSERT must stay pre-`BEGIN IMMEDIATE`** in both `/preview` and `/snapshot/:audit_id/restore`. If anyone moves it back inside the txn, ROLLBACK paths (manual /rollback AND 60s TTL auto-rollback) silently discard the audit row — losing the very visibility audit logging exists for. This was a real bug found in Phase H Test 19 and fixed in `7d3e847`. The fix is documented in the function comment at `sqlConsole.js` ~line 1138.
  - **`invalidateReadonlyDb()` must run after every successful commit.** Phase 1's separate readonly file handle holds a stale WAL read-snapshot across writes made on the writable handle. Without invalidation, `/execute` returns stale data after a write — including the Snapshots dropdown's own SELECT, which the frontend depends on. Defense-in-depth: the readonly file handle still refuses any write attempt at the SQLite level (Phase 1 invariant preserved).
  - **Single-writer invariant** in `txnRegistry` enforced via synchronous "check + placeholder set" pattern. Node's single-threaded JS execution guarantees no race. If anyone refactors `/preview` to add an `await` between the `txnRegistryHasOpen()` check and the `txnRegistry.set(txnId, ...placeholder)` reservation, two concurrent /preview calls could race past the check and both try `BEGIN IMMEDIATE` — second one would `SQLITE_BUSY` but the registry would be inconsistent.
  - **Server kill mid-flight leaves the audit row stuck in `mode='write_preview' status='preview'` forever.** The in-memory TTL timer dies with the process. The DB stays consistent (better-sqlite3 + WAL auto-rollback on connection close), but the audit table accumulates orphan preview rows from any txn that was open at kill time. Phase H Test 21's row (id=5 in the test DB) is the canonical example. Phase 3 should add a startup sweep (`UPDATE sql_console_audit SET status='auto_rolled_back', mode='rolled_back' WHERE status='preview' AND ts < datetime('now', '-1 minute')`).
  - **`writeRateLimiter` counter is in-memory.** Restart resets it. Not a security bug — admin still has to authenticate — but back-to-back deploys reset the per-actor budget.
  - **`SQL_CONSOLE_DDL_TOKEN` must be ≥16 chars** or the boot logs a warning AND `/preview` rejects all DDL attempts with 403. The token VALUE is never logged; only `ddl=on/off` state surfaces in the boot status line.
  - **Snapshot restore needs a primary key.** `getPrimaryKeyColumns` falls back to `['id']` when no PK is declared, then to all columns. None of the 5 protected tables (`employees`, `salary_computations`, `day_calculations`, `attendance_processed`, `salary_structures`) trip this fallback today.
  - **`affected_rows` reflects SQLite's `changes()` post-execute**, not a pre-execute count. For UPDATEs that don't actually modify a value, `changes()` still counts the row. This means the 1000-row hard cap is enforced on "rows touched", not "rows whose value differs" — close enough but worth knowing.
  - **The merge commit `31a2a44` brings 8 source commits onto main in one shot.** Bisecting into the merge will land you on individual phase commits; the merge commit itself is conflict-free (origin/main was at `b52d9db` when the PR was opened; the parallel MCP work landed AFTER the merge so no rebase was needed).
- **Unfinished work:**
  - **Frontend visual smoke on Railway** — sandbox couldn't open a browser. Needs a human to: navigate `/admin/sql-console`, type an UPDATE, see PreviewModal, fill remark, watch DiffView countdown, type confirmRemark in ConfirmModal, see green commit toast. Plus: open Snapshots ▾, click Restore on a row, confirm DiffView re-enters. Plus: with `SQL_CONSOLE_DDL_TOKEN` set, smoke a CREATE → DROP through the modal's DDL token field.
  - **Railway env vars NOT set this session.** Phase 1's `SQL_CONSOLE_ENABLED=true` and `SQL_CONSOLE_API_KEY=<48 hex>` should already be present from the previous session. Phase 2 adds optional `SQL_CONSOLE_DDL_TOKEN=<≥16 chars>` — leaving it unset disables DDL writes which is the safe default.
- **Known issues remaining:**
  - **Orphan-preview cleanup at boot** — see "What's fragile" above. Phase 3 backlog.
  - **`affected_rows` accuracy for UPDATE/DELETE** — currently uses post-execute `.changes`. A row-pre-count via `SELECT COUNT(*) WHERE <whereClause>` would give true intent. Phase 3.
  - **No "snapshots browser" page** — only the toolbar dropdown shows the last 50. Phase 3 if HR ever wants to browse beyond that.
  - **No CSV export from /history** — Phase 1 exports query results only, not the audit log itself. Phase 3.
  - Pre-existing: `[MIGRATION] miss-punch finance queue backfill failed` log on every boot (out of scope).
  - Pre-existing: 5 npm vulnerabilities surfaced by `npm install` (out of scope).
  - Pre-existing: html2pdf chunk 982 kB / xlsx 424 kB / index 380 kB exceed Vite's 500 kB chunk warning (out of scope).
  - Sandbox `/tmp/p2_smoke/` test DB and `/tmp/p2_app.{log,pid}` are leftover from Phase H. Local-only artifacts; Railway unaffected. Cleaned up on next sandbox session.
- **Next session should:** (a) Set `SQL_CONSOLE_DDL_TOKEN` on Railway only if DDL writes are actually needed (default: leave unset, DDL stays disabled). (b) After Railway redeploys with `31a2a44`, check the boot log for both `[SQL_CONSOLE] enabled at /api/admin/sql/*` AND `[SQL_CONSOLE] phase 2 writes: ddl=off ttl=60s row_cap=1000 snapshot_cap=500 rate_limit=10/h protected_tables=5`. (c) Login to `/admin/sql-console` and smoke-test the modal flow with a low-stakes UPDATE on `policy_config` (non-protected) — confirm green commit toast shows audit_id + drift_check=null + snapshot_id=null. (d) Then a small UPDATE on `employees` (protected) for one test row — confirm toast shows snapshot_id + drift_check ran. (e) Open Snapshots ▾ and verify the row appears; click Restore → DiffView shows inverse UPDATE → commit it → confirm employees row reverts. (f) After 24h of Phase 2 use without incident, decide whether to plan Phase 3 (orphan-preview cleanup at boot, snapshots browser page, audit-export CSV).
