...
- **Phase 2 (writes) is NOT in this build.** UPDATE/DELETE/INSERT/DDL all return 403 (`STATEMENT_NOT_ALLOWED`); even if you bypass validation somehow, the readonly file handle returns 403 (`WRITE_BLOCKED_BY_FILE_HANDLE`). Phase 2 will introduce `/preview` → `/commit/:txn_id` → `/rollback/:txn_id` with mandatory remark, 60s TTL, row cap 1000, auto-snapshot for salary tables, and a post-write drift check. Until then this console is strictly read-only by both validation and file handle.
