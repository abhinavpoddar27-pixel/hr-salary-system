...
// ── T3.4: Year-on-Year Payroll Cost Trend ──────────────────
router.get('/salary-trend', (req, res) => {
  try {
    ...
    const startKey  = startYear * 12 + 1;          // January of start year
    ...
    const rows = db.prepare(`
      SELECT
        ...
        COALESCE(SUM(sc.take_home),     0)     AS total_take_home,
        ...
        COALESCE(SUM(sc.ed_pay),        0)     AS total_ed
    ...
    const yearArr = Object.values(byYear).sort((a, b) => a.year - b.year);
    const yearSummaries = yearArr.map((y, idx) => {
      const avgHC = y.headcounts.length > 0
        ? Math.round(y.headcounts.reduce((a, b) => a + b, 0) / y.headcounts.length)
        : 0;
      const avgMonthly = y.months.length > 0 ? Math.round(y.totalNet / y.months.length) : 0;
      const prev = idx > 0 ? yearArr[idx - 1] : null;
      const yoyChange = prev && prev.totalNet > 0
        ? Math.round((y.totalNet - prev.totalNet) / prev.totalNet * 1000) / 10
        : null;
