...
export default function CalendarView({ employeeCode, month, year, data: externalData, compact = false }) {
  ...
  return (
    <div className="inline-block">
      ...
      {/* Legend — always shown, compact in compact mode */}
      <div className={clsx("flex flex-wrap mt-2", compact ? "gap-1 text-[9px]" : "gap-1.5 mt-3 text-[10px]")}>
        {[
          ...
          <span key={l.key} className={`inline-flex items-center gap-0.5 px-1 py-0.5 rounded ${l.color} font-medium`}>
            {compact ? l.key : `${l.key} ${l.label}`}
          </span>
        ))}
        <span className="inline-flex items-center gap-0.5 px-1 py-0.5 text-slate-500">
          <span className="w-1 h-1 bg-amber-500 rounded-full inline-block" /> Late
        </span>
        <span className="inline-flex items-center gap-0.5 px-1 py-0.5 text-slate-500">
          <span className="w-1 h-1 bg-red-500 rounded-full inline-block" /> Miss
        </span>
      </div>
    </div>
  );
